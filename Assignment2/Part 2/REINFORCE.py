...
# ex: return[0] = r0 + gamma*r1 + gamma^2*r2 + ... + gamma^(n-1)*r(n-1)
...
for epi in pbar:
    ...
    for epj in range(EPISODES_PER_EPOCH):
        ...
        # return all states、actions、rewards in an episode
        # S, A, R -> len = n 的 list
        ...
        # all_S = []，用 += 後就不是 mutable object (改 b 不會改到 a)
        all_S += S[:-1] # ignore last state
        all_A += A
        
        # Create returns 
        # discouted_rewards[0] -> 0th action 的 discounted return (r0 + gamma*r1 + gamma^2*r2 + ... + gamma^(n-1)*r(n-1))
        discounted_rewards = copy.deepcopy(R)
        # [start (預設為 0) : stop (預設為 len(R)) : step (一步走多遠，預設為 1)]
        # 如果 start 跟 step 都沒有設定的話，只要 step < 0，他的 start = len(R)，stop = 0
        # 這邊是把原本的 R = [r0, r1, ..., rn] 反過來輸出：r(n-2) -> r(n-3) -> ... -> r1 -> r0
        # range(5) = 1, 2, 3, 4、range(n-1) = 1, 2, ..., n-2
        # 這邊在做的事：
        # R = [r0, r1, r2] -> discounted_rewards = [r0 + gamma*r1 + gamma^2*r2, r1 + gamma*r2]
        for i in range(len(R) - 1)[::-1]:
            discounted_rewards[i] += GAMMA * discounted_rewards[i+1]
        discounted_rewards = t.f(discounted_rewards)  # tensor of float datatype
        all_returns += [discounted_rewards]  # [[return1, return2, ...]]

    Rs += [sum(R)]  # Rs = [total reward of an episode]
    S, A = t.f(np.array(all_S)), t.l(np.array(all_A))
    # returns = [return1, return2, ...] (這幹嘛包成 list 又展回去？)
    # returns 就是 discounted_rewards
...
plt.savefig("/home/super_trumpet/NCKU/Homework/114-2-DRL/Assignment2/Part 2/images/"+args.mode+"/reinforce-"+args.mode+"-10.png")
