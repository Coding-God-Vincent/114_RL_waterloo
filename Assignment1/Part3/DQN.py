...
if __name__ == "__main__":
    ...
    # # Graph1 : Avg. Cumulative Reward of last 25 episodes of 4 differnet sync. duration
    # sync_episodes = [1, 10, 50, 100]
    # for index, se in enumerate(sync_episodes):
    #     print(f"\nsync_episodes = {se}")
    #     curves = []
    #     TARGET_UPDATE_FREQ = se
    #     for seed in SEEDS:
    #         curves.append(train(seed= seed))  # shape (5, 300)
    #     label = "sync. duration = " + str(se)
    #     plt.figure(0)
    #     plot_arrays(curves, colors[index], label)

    #     # if index == 0: curve1 = np.average(curves, axis= 0)
    #     # elif index == 1: curve2 = np.average(curves, axis= 0)
    #     # elif index == 2: curve3 = np.average(curves, axis= 0)
    #     # else: curve4 = np.average(curves, axis= 0)
    
    # # Plot the curve for the given seeds
    # plt.legend(loc='best')
    # plt.title("avg. cumulative reward of the last 25 episodes")
    # plt.xlabel("episodes")
    # plt.ylabel("avg. cumulative reward")
    # plt.legend()
    # plt.savefig('/home/super_trumpet/NCKU/Homework/114-2-DRL/Assignment1/Part3/Avg_Cumulative_reward_last_25_episodes_sync_duration')
    
    # print("\nfinish producing graph1 !\n")
    # Graph2 : Avg. Cumulative Reward of last 25 episodes of 4 differnet mini-batch size
    plt.clf()
    size = [1, 10, 50, 100]
    for index, s in enumerate(size):
        ...
        MINIBATCH_SIZE = s
        for seed in SEEDS:
            curves.append(train(seed= seed))
        label = "minibatch_size = "+ str(s)
        plt.figure(1)
        plot_arrays(curves, colors[index], label)
    ...
    plt.ylabel("avg. cumulative reward")
    plt.legend()
    plt.savefig('/home/super_trumpet/NCKU/Homework/114-2-DRL/Assignment1/Part3/Avg_Cumulative_reward_last_25_episodes_minibatch_size')
    print("\nfinish producing graph2 !")